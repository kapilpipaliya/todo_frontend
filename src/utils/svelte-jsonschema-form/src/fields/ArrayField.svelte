<script>
  ...
  import { getDefaultFormState, isFixedItems, allowAdditionalItems } from '../util';
  ...
  function onReorder(event) {
    ...
    formData = formData.map((item, i, array) => (i === from ? array[to] : i === to ? array[from] : item));
  }
</script>

<Debug
  title="ArrayField"
  data={{ name, schema, formData, uiSchema }} />
