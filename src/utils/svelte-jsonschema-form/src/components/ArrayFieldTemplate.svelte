<script>
  ...
  import { retrieveSchema, toIdSchema } from '../util';
...
<fieldset class={className}>
  <ArrayFieldTitle {idSchema} title={uiSchema['ui:title'] || title} {required} />
  ...
  <div class="row array-item-list">
    {#if items}
      {#each items as item}
        <ArrayItem on:delete-item on:reorder-item bind:itemData={formData[item.index]} {...item} />
      {/each}
    {/if}
  </div>

  {#if canAdd}
    <AddButton on:click={() => dispatch('add-item')} disabled={disabled || readonly} />
  {/if}
</fieldset>
