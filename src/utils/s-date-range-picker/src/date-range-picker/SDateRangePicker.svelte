<script lang="js">
  ...
  css_count.increase('date-range-picker-default');
  css_count.increase('date-range-picker'); // from this component
  css_count.increase('date-range-picker-day');
  ...
  onDestroy(() => {
    ...
    css_count.decrease('date-range-picker-default');
    css_count.decrease('date-range-picker');
    css_count.decrease('date-range-picker-day');
  });
...
<form
  ...
  <label>{startDateReadout()} {!singlePicker ? ` to ${endDateReadout()}` : ''}</label>
  <div class="s-grid">
  ...
  <div class="actions-row">
    ...
    <button
      ...
      disabled={!canApply()}
      on:click={cancel}
      title="Cancel the current selection and revert to previous start and end dates"
      type="button">
      {cancelBtnText}
    </button>
