<script lang="js">
  ...
  $: isOptionDisabled = mo =>
    (!isSameMonth(mo, minDate) && isBefore(mo, minDate)) || (!isSameMonth(mo, minDate) && isAfter(mo, maxDate));
</script>
  ...
  <span>
    {#if monthDropdown}
      <select
        ...
        {#each months as mo}
          <option disabled={isOptionDisabled(mo.value)} selected={isSameMonth(mo.value, month)} value={mo}>{mo.text}</option>
        {/each}
      </select>
    {:else}
    ...
    {#if yearDropdown}
      <select
        ...
        {#each years as yr}
          <option disabled={isOptionDisabled(yr.value)} selected={isSameYear(yr.value, month)} value={yr}>{yr.text}</option>
        {/each}
      </select>
    {:else}
