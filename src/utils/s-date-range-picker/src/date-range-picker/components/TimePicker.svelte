<script lang="js">
  ...
  $: isMinuteOptionDisabled = m => {
    ...
    return (
      (!isSameMinute(date, minDate) && isBefore(date, minDate)) || (!isSameMinute(date, minDate) && isAfter(date, maxDate))
    );
  };
  ...
  $: isSecondOptionDisabled = s => {
    ...
    return (
      (!isSameSecond(date, minDate) && isBefore(date, minDate)) || (!isSameSecond(date, minDate) && isAfter(date, maxDate))
    );
  };
</script>
...
<div class="space-center">
  ...
  <select
    ...
    {#each hours as hour}
      <option value={parseInt(hour)} disabled={isHourOptionDisabled(hour)}>{hour}</option>
    {/each}
  </select>
  <select
    ...
    {#each minutes as minute}
      <option value={parseInt(minute)} disabled={isMinuteOptionDisabled(minute)}>{minute}</option>
    {/each}
  </select>
