<script lang='ts'>
  ...
  onDestroy(S.bindT(form_schema_evt(S.uid), (d) => {if(d[0].routes){r$ = map((x) => modifyObj(x), d[0].routes) } }, ['routes'], 1))
  const modifyObj = o => { 
    ...
    const mod_c = curry((k,o) => {let c = get_c(o[k]); if(c) o[k] = c; return o })
    const mod_g = o => {if(o.onlyIf) {let g = get_g(o.onlyIf.guard); if(g) o.onlyIf.guard = g; } return o }
    const mod_n = o => {if(o.nestedRoutes) o.nestedRoutes = map((x) => modifyObj(x), o.nestedRoutes); return o }
    o = pipe(mod_c('component'), mod_c('layout'), mod_g, mod_n)(o); return o }
</script>
<Notifications>
  <TopLevelComps/>
  <nav>
